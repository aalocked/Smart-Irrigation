    ...
    socket.on("getData", (...args) => {
        ...
        else if(automaticControlMode === 1 && predictionMode === 0)
        {
            ...
            if(M<refValM)
            {
                ...
                //const error = Math.abs(refValM - M)
                let correction = ctr.update(M);
                console.log("correction ", Math.abs(correction));
                const latestCorrection = Math.abs(correction/refValM )*255
                console.log("mcorrection ", latestCorrection);
                io.sockets.emit("receive-motorSpeed-broadcast",latestCorrection);
            }else{
